  ...
  const renderBookmark = (b: Bookmark, index: number) => (
      <div
          ...
          className={`rounded bg-white p-4 shadow dark:bg-gray-800 group flex ${
              view === 'grid' ? 'flex-col h-48 overflow-hidden cursor-pointer' : 'items-start gap-3'
          }`}
                  ...
                  {editingInModal ? (
                      <>
                        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
                          ...
                          <button type="button" onClick={() => { setEditingInModal(false); setSelectedBookmark(null) }} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 p-1"><X size={20} /></button>
                        </div>
