...
fn fill_import_defaults(mut value: serde_json::Value, next_pos: &mut f64) -> serde_json::Value {
    ...
    if let serde_json::Value::Object(ref mut map) = value {
        ...
        // Plain string lists: arrays, JSON text, comma-separated or bare
        // strings all become arrays of trimmed, non-empty strings.
...
fn normalize_string_list(value: serde_json::Value) -> serde_json::Value {
    let entries: Vec<serde_json::Value> = match value {
        serde_json::Value::Array(entries) => entries,
        serde_json::Value::String(text) => {
            let trimmed = text.trim();
