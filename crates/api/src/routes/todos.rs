...
pub async fn create_todo(
    ...
    let next_pos = match data.todo_repo.get_next_position(&user.user_id).await {
        Ok(p) => p,
    ...
    match data.todo_repo.save(&user.user_id, &todo).await {
...
pub async fn update_todo(
    ...
    let existing = match data.todo_repo.find_by_id(&user.user_id, &id).await {
        Ok(Some(t)) => t,
    ...
    match data.todo_repo.save(&user.user_id, &updated).await {
...
pub async fn delete_todo(
    ...
    match data.todo_repo.delete(&user.user_id, &path.into_inner()).await {
...
pub async fn reorder_todos(
    ...
    let positions: Vec<(uuid::Uuid, f64)> = body
        ...
        .collect();
    match data.todo_repo.update_positions(&user.user_id, &positions).await {
