...
pub async fn create_clipboard(
    ...
    let next_pos = match data.clipboard_repo.get_next_position(&user.user_id).await {
        Ok(p) => p,
    ...
    match data.clipboard_repo.save(&user.user_id, &item).await {
...
pub async fn delete_clipboard(
    ...
    match data.clipboard_repo.delete(&user.user_id, &path.into_inner()).await {
...
pub async fn reorder_clipboard(
    ...
    let positions: Vec<(uuid::Uuid, f64)> = body
        ...
        .collect();
    match data.clipboard_repo.update_positions(&user.user_id, &positions).await {
